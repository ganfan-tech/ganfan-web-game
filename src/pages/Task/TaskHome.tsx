...
import { getArticles, getFeed, getTags, request } from '../../services/conduit';
...
import { loadTasks, startLoadingTasks, TaskViewerState } from './task.slice';
...
export function TaskHome() {
  ...
  const { tasks } = useStore(({ taskSlice }) => taskSlice);
...
async function load() {
  ...
  const multipleTasks: MultipleTasks = await request.getTasks();
  // {
  //   tasks: [
  //     {
  //       slug: '1212',
